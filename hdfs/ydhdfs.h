...
#include <iostream>
#include <sstream>
//#include "Global-2.h" //mark relative relationship
using namespace std;
...
struct Writer{
	...
	Writer(const char* path, hdfsFS fs, int me):nxtPart(0),curSize(0)
	{
		this->path=path;
		this->fs=fs;
		this->me=me;
    ...
    void finalize()
    {
		tSize numWritten=hdfsWrite(fs, curHdl, newLine, 1);
        if(numWritten==-1)
		{
			fprintf(stderr, "Failed to write file!\n");
			exit(-1);
...
char* rfind(char* str, char delim)
{
	int len=strlen(str);
	int pos=0;
