...
struct Tree{
	...
	void ConstructTreeIndex(ReadBuffer &rb);
	void ConstructTreeThre(ReadBuffer &rb, int thre);
};
...
void Tree:: ConstructTreeIndex(ReadBuffer &rb) {
	...
	while(!q.empty()) {
		...
		if(nchi == 0){
			if(LeafHead == NULL) //not leaflist but levellist
				LeafHead = tp;
			else 
				LeafHead->next = tp;
		}
...
void Tree:: ConstructTreeThre(ReadBuffer &rb, int thre) {

//	cout << "construct tree thre: " << thre << endl;
	int v, ncand, lev;
	rb.read(&v);
	rb.read(&ncand);
	rb.read(&lev);		
