...
void subpruV(CliNode *back, bool *hav, Tree &T) {
	...
	if(back->chi.size() == 0){
		...
		while(back && i<back->chi.size()){
			...
			if(hav[newback->key]){
				int prevkey = newback->key;
				subpruV(newback, hav, T);
				if(back && i< back->chi.size() && back->chi[i]->key == prevkey)
					++i;
			}
			else ++i;
		}
...
void NodeInsert(int s, int e, int ncand, int *cand, int sldeg, int sgdeg, int *sADJ, int eldeg, int egdeg, int *eADJ) {
	...
	for(int i=0; i<nlts; ++i) {
		...
		if(vto == RANK) {
			if(!loadb[blk]){
				loadb[blk] = true;
				b[blk].loadFromDisk(blkFiles[blk]);
