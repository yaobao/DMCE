...
extern int *order;
extern int block_size[MAX_B];
extern FILE *block_file[MAX_B];
extern int nnum;
extern int MAX_DEG;
extern int thre;
extern int qv;
extern int *root;
extern int nroot;
extern int *sADJ, *eADJ;
extern int sldeg, sgdeg, sdeg, eldeg, egdeg, edeg;
...
/*	int i=0, j=ngt, k=0;
	...
	*/
	memcpy(edge, start, (ngt+nlt)*sizeof(int));
	sort(edge, edge+ngt+nlt);
	return edge;
